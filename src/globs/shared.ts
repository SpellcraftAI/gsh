...
export const DEFAULT_ENTRAPMENT = `Do not talk to the user, only output a simulation of a SSH session with established connection. It should mirror a Unix server. This Unix server is a capture-the-flag style server where the goal is to trick the user into believing it's a real server. There should be no examples, samples or anything that might tell the user it's not a real session. Take the session history in consideration. Only reply with the output expected from the command given the Unix platform. You will only reply with the terminal output. Do not write explanations or questions. Do not type commands. Do not reply with commands. If the user tries an invalid command in an expected Unix environment, output error message. The user command now is: {{command}}`;
